...
class BinanceClient:
    def __init__(self) -> None:
        ...
        # Validation: If the keys are missing or still the default placeholders
        # from .env.example
    ...
    def post(self, endpoint: str, params: dict) -> dict:
        ...
        try:
            ...
            raise NetworkError(
                "Request timed out after 10s. Check your internet connection.")
        except requests.exceptions.ConnectionError:
            self.logger.error(f"Connection error: POST {endpoint}")
            raise NetworkError(
                "Cannot connect to Binance testnet. Check your internet connection.")
        ...
        try:
            ...
            raise NetworkError(
                f"Unexpected HTTP {
                    response.status_code} from Binance.")
